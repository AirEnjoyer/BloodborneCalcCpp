...
weapon::weapon(
  ...
  ) : name(i_name), scales(i_scales), scaleIncrease(i_scaleIncrease), regMults(i_regMults), transformedMults(i_transformedMults), base(i_base), inc(i_inc), finals(i_finals) {}
//   ) {
//     name = i_name;
//     scales = i_scales;
...
std::string weapon::returnName(){
    return name;
}

int weapon::returnScales(int index){
    return scales[index];
}

int weapon::returnScaleIncrease(int index){
    return scaleIncrease[index];
}

int weapon::returnRegMult(int index){
    return regMults[index];
}

int weapon::returnTransformedMult(int index){
    return transformedMults[index];
}

int weapon::returnBase(int index){
    return base[index];
}

int weapon::returnInc(int index){
    return inc[index];
}

int weapon::returnFinals(int index){
    return finals[index];
}
